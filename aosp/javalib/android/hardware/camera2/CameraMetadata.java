...
public abstract class CameraMetadata<TKey> {
  ...
  private static <TKey> boolean shouldKeyBeAdded(TKey paramTKey, Field paramField, int[] paramArrayOfint, boolean paramBoolean) {
    if (paramTKey != null) {
      CameraMetadataNative.Key key;
      if (paramTKey instanceof CameraCharacteristics.Key) {
        key = ((CameraCharacteristics.Key)paramTKey).getNativeKey();
      } else if (key instanceof CaptureResult.Key) {
        key = ((CaptureResult.Key)key).getNativeKey();
      } else if (key instanceof CaptureRequest.Key) {
        key = ((CaptureRequest.Key)key).getNativeKey();
      } else {
        throw new IllegalArgumentException("key type must be that of a metadata key");
      } 
      Object object = paramField.getAnnotation((Class)PublicKey.class);
      boolean bool = false;
      if (object == null)
        return false; 
      ...
      if (paramField.getAnnotation(SyntheticKey.class) != null)
        return paramBoolean; 
      paramBoolean = bool;
      if (Arrays.binarySearch(paramArrayOfint, key.getTag()) >= 0)
        paramBoolean = true; 
      return paramBoolean;
    } 
  ...
  <TKey> ArrayList<TKey> getKeys(Class<?> paramClass, Class<TKey> paramClass1, CameraMetadata<TKey> paramCameraMetadata, int[] paramArrayOfint, boolean paramBoolean) {
    Class<?> clazz = paramClass;
    if (paramClass.equals(TotalCaptureResult.class))
      clazz = CaptureResult.class; 
    if (paramArrayOfint != null)
      Arrays.sort(paramArrayOfint); 
    ArrayList<Object> arrayList1 = new ArrayList();
    for (Field field : clazz.getDeclaredFields()) {
      if (field.getType().isAssignableFrom(paramClass1) && (field.getModifiers() & 0x1) != 0)
        try {
          Object object = field.get(paramCameraMetadata);
          if ((paramCameraMetadata == null || paramCameraMetadata.getProtected((TKey)object) != null) && shouldKeyBeAdded(object, field, paramArrayOfint, paramBoolean))
            arrayList1.add(object); 
        } catch (IllegalAccessException illegalAccessException) {
          throw new AssertionError("Can't get IllegalAccessException", illegalAccessException);
        } catch (IllegalArgumentException illegalArgumentException) {
          throw new AssertionError("Can't get IllegalArgumentException", illegalArgumentException);
        }  
    } 
    CameraMetadataNative cameraMetadataNative = this.mNativeInstance;
    if (cameraMetadataNative == null)
      return (ArrayList)arrayList1; 
    ArrayList arrayList = cameraMetadataNative.getAllVendorKeys(paramClass1);
    if (arrayList != null)
      for (Class<TKey> paramClass1 : (Iterable<Class<TKey>>)arrayList) {
        String str;
        long l;
        if (paramClass1 instanceof CaptureRequest.Key) {
          str = ((CaptureRequest.Key)paramClass1).getName();
          l = ((CaptureRequest.Key)paramClass1).getVendorId();
        } else if (paramClass1 instanceof CaptureResult.Key) {
          str = ((CaptureResult.Key)paramClass1).getName();
          l = ((CaptureResult.Key)paramClass1).getVendorId();
        } else if (paramClass1 instanceof CameraCharacteristics.Key) {
          str = ((CameraCharacteristics.Key)paramClass1).getName();
          l = ((CameraCharacteristics.Key)paramClass1).getVendorId();
        } else {
          continue;
        } 
        if ((paramArrayOfint == null || Arrays.binarySearch(paramArrayOfint, CameraMetadataNative.getTag(str, l)) >= 0) && (paramCameraMetadata == null || paramCameraMetadata.getProtected((TKey)paramClass1) != null))
          arrayList1.add(paramClass1); 
      }  
    return (ArrayList)arrayList1;
  }
  ...
  public List<TKey> getKeys() {
    ...
    return Collections.unmodifiableList(getKeys(clazz, getKeyClass(), this, null, true));
  }
  
  public long getNativeMetadataPtr() {
    CameraMetadataNative cameraMetadataNative = this.mNativeInstance;
    return (cameraMetadataNative == null) ? 0L : cameraMetadataNative.getMetadataPtr();
  }
  ...
  protected void setNativeInstance(CameraMetadataNative paramCameraMetadataNative) {
    this.mNativeInstance = paramCameraMetadataNative;
  }
}


/* Location:              /home/chun/Desktop/temp/!/android/hardware/camera2/CameraMetadata.class
